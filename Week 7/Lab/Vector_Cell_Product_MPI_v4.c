...
// Use non blocking MPI Isend only during data distrution by root process 
// Demonstrates a mix between blocking and non blocking communication
//
//////////////////////////////////////////////////////////////////////////
#include <stdio.h>
#include <stdlib.h>
...
int main()
{
	...
	if(my_rank == 0)
	{
		...
		printf("Rank: %d. MPI Implementation version 4. Commence Reading\n", my_rank);
	...
	if(my_rank == 0){
		...
		for(i = 1; i < p; i++){
			// Only apply non-blocking send when distributing the data to other processes.
			// As soon as the root is done calling MPI_Isend, the root can perform computation on its portion of data.
			// This should marginally improve the performance of the parallel program.
			if(i != p-1){
