	...
	if(my_rank == 0){
		...
		for(i = 1; i < p; i++){
			...
			if(i != p-1){
				...
				MPI_Send((int*)pArrayNum1 + offset, (elementsPerProcess + elementsPerProcessRemain), MPI_INT, i, 0, MPI_COMM_WORLD); 
				MPI_Send((int*)pArrayNum2 + offset, (elementsPerProcess + elementsPerProcessRemain), MPI_INT, i, 0, MPI_COMM_WORLD);	
			}
			offset += elementsPerProcess;
		}
	}else{
